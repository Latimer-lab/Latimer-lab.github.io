...
class ThemeManager {
    ...
    generateRandomDarkColor() {
        ...
        const s = 85; // Fixed saturation 85%
        const l = 8;  // Fixed lightness 8% (less dark)
        return { h, s, l };
    }
    ...
    updateMonacoThemes() {
        ...
        if (window.modalEditor && window.modalEditor.updateOptions) {
            try {
                ...
                // Modal editor might not be fully initialized yet
            }
        }
    }
}
...
class Timer {
    ...
    updateTimer() {
        ...
        // Calculate time until next 8-hour reset
        const nextResetHour = Math.floor(utcHours / 8) * 8 + 8;
        const hoursUntilReset = nextResetHour - utcHours;
        const minutesUntilReset = 59 - utcMinutes;
        const secondsUntilReset = 59 - utcSeconds;

        // Format the timer display
        const hours = hoursUntilReset.toString().padStart(2, '0');
        const minutes = minutesUntilReset.toString().padStart(2, '0');
        const seconds = secondsUntilReset.toString().padStart(2, '0');
